...
#include <EGL/egl.h>
#include "windowing/egl/EGLNativeTypeAndroid.h"
...
public:
  ...
  virtual int       kodi_get_duration() = 0;
...
class DllLibRKCodec : public DllDynamic, DllLibRKCodecInterface
{
  ...
  DEFINE_METHOD0(int,     kodi_get_duration);
  ...
  BEGIN_METHOD_RESOLVE()
    ...
    RESOLVE_METHOD(kodi_get_duration)
...
bool CRKCodec::OpenDecoder(CDVDStreamInfo &hints)
{
  ...
  if (!err)
  {
    ...
    CLog::Log(LOGDEBUG,"CRKCodec::kodi_opencodec() width = %d , height = %d, sWidth = %d, sHeight = %d, bFullStreen = %d, pixfmt = %d", 
...
bool CRKCodec::GetPicture(DVDVideoPicture *pDvdVideoPicture)
{
  ...
  pDvdVideoPicture->iDuration = m_iDuration;
  pDvdVideoPicture->dts = DVD_NOPTS_VALUE;
  pDvdVideoPicture->pts = DVD_NOPTS_VALUE;
  if (m_speed == DVD_PLAYSPEED_NORMAL)
  {
    pDvdVideoPicture->pts = GetPlayerClock() + m_iDuration;
  }
  else
  {
    if (m_cur_pts <= 100)
      pDvdVideoPicture->pts = (double)m_1st_pts;
    else
      pDvdVideoPicture->pts = (double)m_cur_pts;
  }
  return true;
}


double CRKCodec::GetPlayerClock()
{
...
void CRKCodec::Process()
{
  ...
  while(!m_bStop)
  {
    ...
    if (m_1st_pts > 0 || m_1st_pts == DVD_NOPTS_VALUE)
    {
      ...
      if (m_bWaitEos && video_pts == RK_EOS_REACH)
      {
        m_eosCond.notifyAll();
        CLog::Log(LOGDEBUG,"rkcodec eos reach");
        goto SLEEP;
      }
      if (m_cur_pts != video_pts)
      {
        double app_pts = GetPlayerClock();
        m_cur_pts = video_pts;
        ...
        app_pts += offset*DVD_TIME_BASE;
        double error = (double)(app_pts - video_pts)/ DVD_TIME_BASE;
        double abs_error = fabs(error);
        if (abs_error > 0.125)
        {
          if (abs_error > 0.15)
          {
            SetVideoPts((int64_t)app_pts);
          }
          else
          {
        ...
        if (m_iDuration == 0)
          m_iDuration = m_dll->kodi_get_duration();
      }
    }
SLEEP:
    Sleep(100);
...
void CRKCodec::WaitForEosReach(int milliseconds)
{
  CLog::Log(LOGDEBUG, "wait for rk eos reach");
  m_bWaitEos = true;
  CSingleLock lock(m_eosMutex);
  XbmcThreads::EndTime timer(milliseconds);
  m_eosCond.wait(m_eosMutex, milliseconds);
  if (timer.IsTimePast()) 
    CLog::Log(LOGDEBUG, "wait for rk eos timeout");
  else
    Sleep(1000);
}
...
void CRKCodec::UpdateRenderRect(const CRect &SrcRect, const CRect &DestRect)
{
  ...
  if (true || CJNIBase::GetSDKVersion() >= 21)
  {
    /*  in 5.1 */
