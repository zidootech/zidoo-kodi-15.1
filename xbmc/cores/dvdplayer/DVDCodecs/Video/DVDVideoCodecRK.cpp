...
int CDVDVideoCodecRK::Decode(uint8_t *pData, int iSize, double dts, double pts)
{
  ...
  { 
    ...
    m_Codec->Set23976Match(CSettings::GetInstance().GetBool(CSettings::SETTING_VIDEOPLAYER_USERKFPSMATCH));
...
{
  if (flags == -1 && m_Codec)
    return m_Codec->WaitForEosReach(5000);
}
...
void rk_set_codec_enable(int enable)
{
	enableRkCodec = enable;
}
